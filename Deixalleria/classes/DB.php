...
class DB {      // Classe de totes les funcions de l'aplicació
    public function __construct() { // Aseguramos que se carguen las constantes seguras si el archivo existe
        if (file_exists(__DIR__ . '/../configmysql.php')) {
            include_once __DIR__ . '/../configmysql.php';
        }

        // Evaluamos si las constantes están definidas
        $db_host = defined('host') ? host : 'localhost';
        $db_user = defined('user') ? user : 'usuario_demo';
        $db_pass = defined('password') ? password : 'contraseña_demo';
        $db_name = defined('database') ? database : 'deixalleria';

        // Establecemos la conexión utilizando las variables protegidas
